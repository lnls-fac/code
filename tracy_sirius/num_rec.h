...
#define SIGN(a,b) ((b) >= 0.0 ? fabs(a) : -fabs(a))
  ...
  void dmvmult( double **a, int a_rows, int a_cols, double *b, int b_els,
		double *y);
  ...
  void dmdump( FILE *outf, char *text, double **a, int a_rows, int a_cols,
	       char *format);
  ...
  void dReadMatrix(FILE *datafile, double **y, int *rows, int *cols,
		   int *error);
  void dReadVector(FILE *datafile, double *y, int *els, int *error);
  double dReadScalar(FILE *datafile, int *error);
  void dWriteMatrix( FILE *outf, char *text, double **a, int a_rows,
		     int a_cols, char *format);
  void dWriteVector( FILE *outf, char *text, double *a, int a_els,
		     char *format);
  ...
  /* copy a vector */
  void dvcopy( double *a, int a_els, double *b);
  /* compare two vectors */
  double dvcomp( double *a, int a_els, double *b);
  /* get a column from a matrix */
  void dgetcolumn(double **G, int col, double *v, int nels);
  /* put a column into a matrix */
  void dputcolumn(double *v, int nels, double **G, int col);

  void dinverse( double **a, int n, double **y );
  void dinverse_mult( double **a, int a_rows, double **b, int b_cols,
		      double **y );
...
#define _HUFFCODE_DECLARE_T_
  ...
  void period(float x[], float y[], int n, float ofac, float hifac,
	      float px[], float py[], int np, int *nout, int *jmax,
	      float *prob);
  ...
  float select1(unsigned long k, unsigned long n, float arr[]);
  ...
  void sprstm(float sa[], unsigned long ija[], float sb[], unsigned long ijb[],
	      float thresh, unsigned long nmax, float sc[],
	      unsigned long ijc[]);
