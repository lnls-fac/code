...
#define NoBEmax      18000         /* maximum number of elements in
				     a block (Elem_NFam) */
                            ...
                            Lat_symbol *sps_, struct LOC_Lattice_Read *LINK)
{
  ...
  case mpsym:  /*4*/
	...
	WITH2->Pirho = t * M_PI / 180.0;
      ...
      WITH2->PdTpar = dt;
      AssignHOM(globval.Elem_nFam, &V);
      //Introduced by Fernando (05/10/2012) to correct application of multipole errors
      if ( WITH2->Pirho != 0){
    	  WITH2->n_design = Dip;
      }else{
    	  for (i = 0; i <= HOMmax; i++) {
    		  if ((WITH2->PBpar[i+HOMmax] != 0) || (WITH2->PBpar[-i+HOMmax] != 0)) {
    			  WITH2->n_design = i;
    			  break;
    		  }
    	  }
...
//      WITH2->n_design = WITH2->Porder;
  ...
  case wglsym:
    ...
    *************************************************************************
    ...
    Example
      ...
      FM: Fieldmap, L = 1.0, N = 20, file1 = "U19_Bxyz.dat";
  ...
  case fmsym:
    ...
    if (globval.Elem_nFam <= Elem_nFamMax) {
      ...
      FieldMap_Alloc(WITH1, true);
      WITH6 = WITH1->FM; WITH6->n_step = k1;
...
/* void GetDP(struct LOC_Lattice_Read *LINK)
   ...
   Purpose:
       Define particle energy offset read from lattice file
...
/****************************************************************************/
/* void GetCODEPS(LOC_Lattice_Read *LINK)

   Purpose:
       Read and assign cod precision read from lattice file
...
/****************************************************************************/
/* Local double Circumference(struct LOC_Lattice_Read *LINK)
...
/****************************************************************************/
/* Local void RegisterKids(struct LOC_Lattice_Read *LINK)
...
long ElemIndex(const char *name)
{
  ...
  strcpy(name1, name);
// trecho comentado para permitir uso de letras maiúsculas para familias 2013-03-07
...
/****************************************************************************/
/* boolean Lattice_Read(FILE **fi_, FILE **fo_)

   Purpose:
       Low level routine for reading lattice file
