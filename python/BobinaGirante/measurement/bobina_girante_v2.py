...
class JanelaGrafica(QtGui.QMainWindow):
    ...
    def load_default(self):
        ...
        self.ui.premont_A.setText(config[4][8][1])
        self.ui.premont_B.setText(config[4][9][1])
        self.ui.final_A.setText(config[4][10][1])
        self.ui.final_B.setText(config[4][11][1])
        self.ui.pos_ang.setText(config[4][12][1])
        self.ui.pos_long.setText(config[4][13][1])
        self.ui.pos_ver.setText(config[4][14][1])
        self.ui.pos_trac.setText(config[4][15][1])
...
##            self.ui.Tempo_Auto.setText(dados[8])
    ...
    def Corrente_Fonte(self):
        try:
            corrente = round(lib.PUC.ReadAD(),3)
            fator_entrada = float(self.ui.Fator_Entrada.text())
            self.ui.lcd_Corrente.display((corrente*fator_entrada))
        except:
...
##            self.ui.Configuracao_Manual.setEnabled(True)
    ...
    def Plotar_Curva(self, pontos):
        curva, checksum, pontos, ciclos, freq = self.Gerar_Curva_Senoidal()
    ...
    def Ciclagem(self):
        try:
            ...
            QtGui.QMessageBox.warning(self,'Atenção.','Placa PUC não esta Respondendo.',QtGui.QMessageBox.Ok)
            return
        try:
            f_saida = float(self.ui.Fator_Saida.text())
            ...
            tempo = float(self.ui.Tempo_Linear.text())
            freq = float(self.ui.Frequencia_Senoidal.text())
            corrente_final = (float(self.ui.Corrente_Final_Senoidal.text()))/f_saida
        ...
        n = int(ciclos * rep_float)
        ponto_final = (int((lib.Ciclos_Puc/ciclos)*n)+((lib.Ciclos_Puc*3)/(ciclos*4)-(75)))
        ####################
        
        self.Rampa(final,atual,passo,tempo)
        lib.PUC.ExecuteCurve(lib.Ciclos_Puc, 0, 0, 1, 0, 0, 1)
...
def ColetaDados(Repeticao):
    ...
    while coleta == False and lib.parartudo == 0:
        ...
        if i>2:
            QtGui.QMessageBox.warning(lib.Janela,'Aviso.','Erro muito Alto. Processo Finalizado.\nVerifique parametros e equipamentos.',QtGui.QMessageBox.Ok)
            lib.Janela.ui.groupBox_2.setEnabled(True)
            lib.Janela.ui.coletar.setEnabled(True)
            return
        i+=1
