...
class MyForm(QtGui.QMainWindow):
  ...
  def multi_norm_calc(self):
    ...
    for m in self.n_multi:
        ...
        if self.spec_data[m - 1] > 0: #positivo vermelho especificação
          self.bar_color_spec.append('r')
        else:
          self.bar_color_spec.append('g')

        if self.sim_data[m - 1] > 0: #positivo vermelho simulação
          self.bar_color_sim.append('r')
        else:
          self.bar_color_sim.append('g')
    ...
    if self.count_file == 0:
      rects_spec = self.ax1.bar(ind_spec, np.absolute(self.spec_data), self.width, bottom=self.bottom_limit, color=self.bar_color_spec, log='y', yerr=[y_lower_spec,self.spec_data_std], ecolor='b')
      rects_sim = self.ax1.bar(ind_sim, np.absolute(self.sim_data), self.width, bottom=self.bottom_limit, color=self.bar_color_sim, log='y')
  ...
  def multi_skew_calc(self):
    ...
    self.spec_data_skew = np.ones(self.n_multi[-1]) * self.bottom_limit * 10 ** (-3)
    ...
    # corrigindo o limite inferior das barras de erro da especificacao
    for i in self.n_multi: 
      if self.bottom_limit>(np.absolute(self.spec_data_skew[i-1])-np.absolute(self.spec_data_skew_std[i-1])):
        bottom_limit_= np.absolute(self.spec_data_skew[i-1])
        y_lower_spec = np.append(y_lower_spec, bottom_limit_)
      else:
    ...
    if self.count_file == 0:
      rects_spec_skew = self.ax2.bar(ind_spec, abs(self.spec_data_skew), self.width, bottom=self.bottom_limit, yerr= [y_lower_spec, np.absolute(self.spec_data_skew_std)], log='y')
  ...
  def main(self):
    ...
    for self.count_file in range(self.n_file):
      ...
      if self.ima == 1: # quadrupolo defocalizador
        ...
        self.spec_data_skew_std =  [0, 0,         1.00*10**(-4),  5.00*10**(-4),  1.00*10**(-4),  1.00*10**(-4),    1.00*10**(-4),  1.00*10**(-4),  1.00*10**(-4),  1.00*10**(-4),    0,              0,              0,              1.00*10**(-4),    0]
        self.spec_data_std =       [0, 0,         7.00*10**(-4),  4.00*10**(-4),  4.00*10**(-4),  4.00*10**(-4),    4.00*10**(-4),  4.00*10**(-4),  4.00*10**(-4),  4.00*10**(-4),    0,              0,              0,              4.00*10**(-4),    0]
