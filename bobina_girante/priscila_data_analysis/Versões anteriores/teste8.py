...
class MyForm(QtGui.QMainWindow):
  ...
  def multi_norm_calc(self):
    ...
    # barras de medicao 
    rects1 = self.ax1.bar(self.ind, abs(self.multi_norm_normalized), self.width, bottom=self.bottom_limit, color=self.bar_color, log='y', yerr=[y_lower, abs(self.multi_norm_normalized_std)], ecolor='b')
    ...
    if self.count_file == (self.n_file/2 - 1):
      self.autolabel(rects1,self.multi_norm_normalized_std)
  ...
  def autolabel(self,rects,multi_norm_normalized_stds):
      ...
      for rect in rects:
          ...
          self.ax1.text(rect.get_x()+rect.get_width()/2., 1.1*(height+multi_norm_normalized_stds[i]),'M', ha='center', va='bottom', fontsize=13)
          i+=1
  ...
  def autolabel_spec_sim(self,rects_spec_sim, spec_data_stds, labels):
      ...
      for rect_spec_sim in rects_spec_sim:
          ...
          self.ax1.text(rect_spec_sim.get_x()+rect_spec_sim.get_width()/2., 1.1*(height+spec_data_stds[i]), labels, ha='center', va='bottom', fontsize=13)
          i+=1
  ...
  def multi_skew_calc(self):
    ...
    # barras de medicao 
    rects2 = self.ax2.bar(self.ind2, abs(self.multi_skew_normalized), self.width, bottom=self.bottom_limit, color=self.bar_color, log='y', yerr=[y_lower, abs(self.multi_skew_normalized_std)], ecolor='b')
    ...
    if self.count_file == (self.n_file/2 - 1):
      self.autolabel_skew(rects2, self.multi_skew_normalized_std)
  ...
  def autolabel_skew(self,rects,multi_skew_normalized_stds):
      ...
      for rect in rects:
          height = rect.get_height()
          self.ax2.text(rect.get_x()+rect.get_width()/2., 1.1*(height+multi_skew_normalized_stds[i]),'M', ha='center', va='bottom', fontsize=13)
          i += 1
  ...
  def autolabel_spec_sim_skew(self,rects_spec_skew, spec_data_skew_stds, labels):
      ...
      for rect_spec_skew in rects_spec_skew:
          ...
          if height != 0:
            self.ax2.text(rect_spec_skew.get_x()+rect_spec_skew.get_width()/2., 1.1*(height+spec_data_skew_stds[i]), labels, ha='center', va='bottom', fontsize=13)
          i += 1
  ...
  def resid_norm_calc(self):
    ...
    for i in range(self.rcb * 2 + 1):
        ...
        for m in self.n_multi:
          if y != 0:
            B_resid += self.multi_norm[m - 1] * pos[i] ** (m - 1)
            z = (B_resid - self.multi_norm[self.ima] * pos[i] ** self.ima - self.multi_norm[0]) / (self.multi_norm[self.ima] * pos[i] ** self.ima)
            
            B_resid_sim += self.sim_data_multi[m - 1] * pos[i] ** (m - 1)
            w = (B_resid_sim - self.sim_data_multi[self.ima] * pos[i] ** self.ima) / (self.sim_data_multi[self.ima] * pos[i] ** self.ima)

            B_spec_resid += self.spec_data[m - 1] / self.r0 ** (m - self.ima + 1) * pos[i] ** (m - self.ima + 1) # calculo do campo residual a partir dos multipolos normalizados
          else:
            z = 0
            w = 0
            B_espec_resid = 0
    ...
    plt.plot(pos, B_resid_norm, label = 'I = %.2f A'% self.corr_alim_princ, linewidth=2.0)
   
    plt.xlabel('Transversal Position x [m]', fontsize=20)
    if self.ima == 1:
      plt.ylabel(r'$\frac{\sum_{n=3}^{'+self.n_multi_str[-1]+'}B_{N_n}}'+'{B_{N_'+str(self.ima+1)+'}}$', fontsize=30)
    else:
      plt.ylabel(r'$\frac{\sum_{n=2-[n='+str(self.ima+1)+']}^{'+self.n_multi_str[-1]+'}B_{N_n}}'+'{B_{N_'+str(self.ima+1)+'}}$', fontsize=30)
