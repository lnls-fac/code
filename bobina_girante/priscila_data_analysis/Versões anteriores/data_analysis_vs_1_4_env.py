...
class MyForm(QtGui.QMainWindow):
  ...
  def multi_skew_calc(self):
    ...
    for m in self.n_multi:
        self.multi_normalized = (self.multi_skew[m-1] - self.multi_skew_env[m-1]) * self.r0 ** (m-1) / ((self.multi_norm[self.ima] - self.multi_skew_env[self.ima]) * self.r0 ** self.ima)
        self.multi_skew_normalized = np.append(self.multi_skew_normalized, float(self.multi_normalized))

        self.multi_normalized_std = (self.multi_skew_std[m-1] + self.multi_skew_std_env[m-1]) * self.r0 ** (m-1) / ((self.multi_norm[self.ima] - self.multi_norm_env[self.ima]) * self.r0 ** self.ima)
  ...
  def resid_norm_calc(self):
    ...
    for i in range(int(self.rcb * 4) + 1):
        ...
        for m in self.n_multi:
          if y != 0:
            if self.ima != 0:  
                B_resid += (self.multi_norm[m - 1] - self.multi_norm_env[m - 1]) * self.pos[i] ** (m - 1)
                z = (B_resid - (self.multi_norm[self.ima] - self.multi_norm_env[self.ima]) * self.pos[i] ** self.ima - (self.multi_norm[0] - self.multi_norm_env[0])) / ((self.multi_norm[self.ima] - self.multi_norm_env[self.ima]) * self.pos[i] ** self.ima)
                ...
                B_resid += (self.multi_norm[m - 1] - self.multi_norm_env[m - 1]) * self.pos[i] ** (m - 1)
                z = (B_resid - (self.multi_norm[0] - self.multi_norm_env[0])) / (self.multi_norm[0] - self.multi_norm_env[0])
        ...
        for m in self.n_multi:
          if y != 0:
            if (2 * m) <= self.n_multi[-1]:
                B_resid_cv += (-1) ** (m) * (self.multi_norm[2 * m] - self.multi_norm_env[2 * m]) * self.pos[i] ** (2 * m) / (self.multi_norm[0] - self.multi_norm_env[0])
  ...
  def excit_calc(self):
    ...
    self.B_excit = np.append(self.B_excit, (self.multi_norm[self.ima] - self.multi_norm_env[self.ima]))
    self.B_excit_std = np.append(self.B_excit_std, (self.multi_norm_std[self.ima] + self.multi_norm_std_env[self.ima]))
