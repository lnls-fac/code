...
class MyForm(QtGui.QMainWindow):
  ...
  def multi_norm_calc(self):
    ...
    if count_file == 0:
      ...
      self.autolabel_spec_sim(rects_spec, spec_data_std, 'E')
      self.autolabel_spec_sim(rects_sim, sim_data_std, 'S')
      
    if count_file == (n_file/2 - 1):
      self.autolabel(rects1,multi_norm_normalized_std)
  ...
  def autolabel(self,rects,multi_norm_normalized_stds):
      i=0
      for rect in rects:
  ...
  def multi_skew_calc(self):
    ...
    if count_file == 0:
      ...
      self.autolabel_spec_sim_skew(rects_spec_skew, spec_data_skew_std, 'E')
      
    if count_file == (n_file/2 - 1):
      self.autolabel_skew(rects2, multi_skew_normalized_std)
  ...
  # nomeando o topo da barra de medicao central
  def autolabel_skew(self,rects,multi_skew_normalized_stds):
      i = 0
      for rect in rects:
          height = rect.get_height()
  ...
  def excit_calc(self):
    ...
    global B_excit, B_excit_std, corr_alim_princ_arr, corr_alim_princ_arr_std, count_file, B_excit_ajust, fig4, fig5, name_fig, B_excit_ajust_sim
    ...
    if count_file == n_file - 1:
      ...
      p1, = plt.plot(sim_corr_sat, sim_data_sat, marker ='^',markersize=3, linewidth=2.0)
      p2 = ax4.errorbar(corr_alim_princ_arr, B_excit, xerr=corr_alim_princ_arr_std, yerr=B_excit_std, marker ='o',markersize=3, linewidth=2.0)

      # calculando ajuste linear
      params_sim = np.polyfit(sim_corr_sat, sim_data_sat, 1)
      [a_sim, b_sim] = params_sim
      B_excit_ajust_sim = a_sim * sim_corr_sat + b_sim
      p3, = plt.plot(sim_corr_sat, B_excit_ajust_sim, '-y', linewidth=1.0)
      
      params = np.polyfit(corr_alim_princ_arr, B_excit, 1)
      [a, b] = params
      B_excit_ajust = a * corr_alim_princ_arr + b
      p4, = plt.plot(corr_alim_princ_arr, B_excit_ajust, '-r', linewidth=1.0)
     
      # colocando dados do ajuste no grafico
      if ima == 0:
        ax4.text(1.5, 2.5, '\n Integrated Field = A * Excitation Current + B \n\n Curve Fitting Line - Simulation \n A = %.4e\n B = %.4e\n\n Curve Fitting Line - Measurement \n A = %.4e\n B = %.4e\n' % (a_sim, b_sim, a, b), color='black', fontsize=20,
      ...
      if ima == 1:
        plt.ylabel('$N_{'+str(ima+1)+'}$ [T]', fontsize=30)
      elif ima == 2:
        plt.ylabel('$N_{'+str(ima+1)+'}$ [T/m]', fontsize=30)
      else:
        plt.ylabel('$N_{'+str(ima+1)+'}$ [T.m]', fontsize=30)
      ...
      B_excit_resid_sim = (sim_data_sat - B_excit_ajust_sim) / sim_data_sat * 100
      sim_corr_sat_ = sim_corr_sat
      ...
      B_excit_resid_sim_sat = B_excit_resid_sim
      plt.plot(sim_corr_sat_, B_excit_resid_sim_sat, marker='^', color='b', label = 'Simulation', linewidth=2.0)
      
      B_excit_resid_sat = np.delete(B_excit_resid,0)
      plt.plot(corr_alim_princ_arr_sat, B_excit_resid_sat, marker='o', color='r',  label = 'Measurement', linewidth=2.0)

      plt.xlabel('Excitation Current [A]', fontsize=20)

      plt.ylabel(r'$\frac{N_{'+str(ima+1)+'} - N_{'+str(ima+1)+'_{Adjusted}}}{N_{'+str(ima+1)+'_{Adjusted}}}$'' [%]', fontsize=30)
  ...
  def tabela(self):
    ...
    ax7.set_title(nome_ima +' - '+data+'- Normal and Skew Integrated Multipoles - Current - %.2f A' % corr_alim_princ, fontsize=20)
  ...
  def main(self):
    global n_file, count_file, r0, rcb, fig1, fig2, fig3, fig4, fig5, fig6, sim_data, spec_data, spec_data_std, width, B_excit, B_excit_std, B_excit_ajust,\
           corr_alim_princ_arr, corr_alim_princ_arr_std, sim_data_multi, ind, ind2, bottom_limit_array, bottom_limit, B_excit_ajust, name_fig, fig7, sim_corr_sat, sim_data_sat, sim_data_std, spec_data_skew_std, ang_arr, ang_arr_std
    ...
    for count_file in range(n_file):
      self.import_data (self.path1 + path_abs1_all[count_file])

      if ima == 0: # corretora horizontal sobre retas, especificacao homogeneidade 1/300 em +/' 15 mm
        rcb = 15
        r0 = 15
        r0 = r0 / 1000
        ...
        sim_data_sat =                 [1.50754*10**(-3), 3.02946*10**(-3)]
        sim_corr_sat =        np.array([4.5,              9               ])
      ...
      if ima == 1: # quadrupolo defocalizador
        ...
        sim_data_multi =      [0, 4.69579,  0,              0,              0,              -5.22255*10**(4), 0,              0,              0,              6.30039*10**(11), 0,              0,              0,              1.57127*10**(17), 0]

        sim_data_sat =                 [7.40767*10**(-1), 2.03711, 2.35045, 2.96292, 4.07253, 4.69579]
        sim_corr_sat =        np.array([20,               55,      63.46,   80,      110,     126.92 ])
      ...
      if ima == 2: # sextupolo focalizador
        ...
        sim_data_sat =                 [-9.95802, -11.5278, -19.9160]
        sim_corr_sat =        np.array([4.06,     4.7,      8.12    ])
    ...
    for k in range (7):
      fig[k].savefig(self.path1+name_fig[k]+'.png')
