...
%  Modified by Ximenes Resende (to print element index)
...
fprintf(fid,'   IDKICKTABLE POS  LENGTH  NY NX\n');
fprintf(fid,'\n');

L = findspos(THERING, 1:length(THERING));

for i=index
    elstring = [sprintf('%04i %11s ',i, THERING{i}.FamName) ' '];
    switch THERING{i}.PassMethod
        case {'DriftPass'}
            elstring = [elstring sprintf('%11s %11.6f %10.7f\n','DRIFT', L(i), THERING{i}.Length)];
        case {'IdentityPass'}
            elstring = [elstring sprintf('%11s %11.6f %10.7f\n','MARKER', L(i),THERING{i}.Length)];
        case {'AperturePass'}
            elstring = [elstring sprintf('%11s %11.6f\n','Aperture', L(i))];
        case {'CavityPass'}
            elstring = [elstring sprintf('%11s %11.6f %10.7f %10.1f\n',...
                'CAVITY', L(i), THERING{i}.Length, THERING{i}.Frequency)];
        case {'CorrectorPass'}
            elstring = [elstring sprintf('%11s %11.6f %10.7f %10.7f %10.7f\n',...
                'CORRECTOR', L(i), THERING{i}.Length, THERING{i}.KickAngle)];
        case {'QuadLinearPass'}
            elstring = [elstring sprintf('%11s %11.6f %10.7f %10.7f\n',...
                'QUAD', L(i), THERING{i}.Length,THERING{i}.PolynomB(2))];
        case {'FFQuadLinearPass'}
            elstring = [elstring sprintf('%11s %11.6f %10.7f %10.7f\n',...
                'QUAD', L(i), THERING{i}.Length,THERING{i}.K)];
        case {'StrMPoleSymplectic4Pass' 'StrMPoleSymplectic4RadPass'}
            elstring = [elstring sprintf('%11s %11.6f %10.7f ','MULTIPOLE', L(i), THERING{i}.Length) ...
                sprintf('%10.7f ', THERING{i}.PolynomB) sprintf('\n')];
        case {'BendLinearPass' 'BndMPoleSymplectic4Pass','BndMPoleSymplectic4RadPass'}
            elstring = [elstring sprintf('%11s %11.6f %10.7f %10.7f %10.7f %10.7f ',...
                'BEND', L(i), THERING{i}.Length,THERING{i}.BendingAngle, THERING{i}.EntranceAngle, THERING{i}.ExitAngle) ...
                sprintf('%10.7f ', THERING{i}.PolynomB) sprintf('\n')];
        case {'LNLSThinEPUPass'}
             elstring = [elstring sprintf('%11s %11.6f %10.7f ','IDKICKTABLE', L(i), THERING{i}.Length) ...
                sprintf('%i %i ', size(THERING{i}.PxGrid,1), size(THERING{i}.PxGrid,2)) sprintf('\n')];
        otherwise
            elstring = [elstring sprintf('%11s %11.6f (#%d)\n', 'UNKNOWN ELEMENT', L(i), i')];
...
if input
    ...
    for i=1:length(elementline)
        elstring = [elstring sprintf('%11s ',elementline{i})];
        if mod(i,5) == 0
            fprintf(fid,'%s\n',elstring);
            elstring = '';
        end
