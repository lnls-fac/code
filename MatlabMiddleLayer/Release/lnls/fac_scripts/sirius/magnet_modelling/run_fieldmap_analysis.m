function r = run_fieldmap_analysis(config_file)
...
if isfield(parms,'magnet_subtype')
    load_fieldmap(parms.fmap_fname, parms.magnet_subtype);
else
...
if ~isempty(parms.rk_traj)
    ...
    [~, zmax, xmin, xmax] = get_fmaps_boundingbox; s_length = ...
                    max([zmax + (xmax - xmin), 1.01*parms.model.half_length]);
    rk_traj = calc_trajectory(s_length, parms.init_position, ...
                              parms.runge_kutta_flags);
...
rk_traj_parms = calc_parameters_on_rk_trajectory(rk_traj, parms.beam, ...
                  parms.magnet_type, parms.tracy.r0, parms.perp_grid.monomials);

% creates segmentated model based on PolynomB profil
%seg_model = generate_model_segmentation(rk_traj, parms.model.half_length, ...
%                 parms.perp_grid.monomials, 'load_return', parms.config_path);
seg_model = generate_model_segmentation(rk_traj, parms.model.half_length, ...
                          parms.perp_grid.monomials, 'load', parms.config_path);

% creates segmented AT model (with thin element bumping all multipoles 
%  outside model half-length)
at_model  = create_at_model(rk_traj, seg_model, parms.magnet_type, ...
                            parms.perp_grid.monomials, parms.nominal_ang);

% creates reference trajectory path based on ref_traj defined by AT model
ref_traj  = gen_ref_traj_from_rk_traj(rk_traj, seg_model.s(end), ...
                                      parms.magnet_type, parms.nominal_ang);

% tracks particle through fieldmap and generates kick curves
fieldmap_track = track_through_fieldmap(ref_traj, parms.track, ...
                                        parms.runge_kutta_flags);

% calibrates at_model to yield kick curve close to the one from the fieldmap
[at_model_calibrated, M_fieldmap, M_atmodel, K_fieldmap, K_atmodel] = ...
    calibrate_at_model(ref_traj, at_model, fieldmap_track, parms.track, ...
                       parms.perp_grid.monomials, parms.runge_kutta_flags, ...
                       parms.calibration);
atmodel_track = track_through_atmodel(at_model_calibrated, parms.track, ...
                                      ref_traj.s(end));


% prints summary data
print_summary(parms, rk_traj, rk_traj_parms, ref_traj, at_model_calibrated, ...
              fieldmap_track, atmodel_track, M_fieldmap, M_atmodel);
