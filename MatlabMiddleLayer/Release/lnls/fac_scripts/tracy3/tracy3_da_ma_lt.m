...
%     if nr_rms == 0, nr_rms = n_pastas; end
    ...
    cell_leg_text(i) = inputdlg(['Digite a legenda'],'Legenda',1);
    ...
    for k=1:n_pastas; %min([n_pastas, nr_rms]);
        ...
        %malt
        try
            [spos, accep(l+1,:,:), nLost(l+1,:,:), sLost(l+1,:,:)] = tracy3_load_ma_data(pathname);
            l = l + 1;
        catch
            fprintf('%-2d-%-3d: ma nao carregou',i,k);
        end
        Accep(1,:) = spos;
        Accep([2 3],:) = min(accep(l,:,:), accepRF);
