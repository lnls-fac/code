function r = lnls1_measeputable(varargin)
...
r.tolerance = 200e-3;     % tolerancia em mm (movimentações do EPU)
...
r.pause_after_epu_setpoint = 12;
r.delta_respm = 0.5; % amp

% r.gaps  = unique([22:2.5:97 100:50:300]); % Grid original de 2010
% r.fases = unique([-25:5:25]);

r.gaps  = unique([22:2.5:32]);
r.fases = unique([-25:5:25]);
...
%setpv('AON11', 'PhaseSpeed', 500);
%setpv('AON11', 'GapSpeed', 500);
...
setpv('AOV11A_SP',0);
setpv('AOV11B_SP',0);
IDS_REF(1).channel_name = 'AON11FASE';
...
pause(5.0);
lnls1_set_id_configurations(IDS_REF(1));
pause(r.pause_after_epu_setpoint);
lnls1_set_id_configurations(IDS_REF(2));
pause(r.pause_after_epu_setpoint);
...
for j=1:length(r.gaps)
    ...
    for i=1:length(r.fases)
        ...
        %epu_go(r.gaps(j), r.fases(i), r.tolerance);
        IDS(1).value = r.fases(i);
        IDS(2).value = r.gaps(j);
                 
        pause(5.0);
        lnls1_set_id_configurations(IDS(1));
        pause(r.pause_after_epu_setpoint);
        lnls1_set_id_configurations(IDS(2));
        pause(r.pause_after_epu_setpoint);
...
% function epu_go(ref_gap, ref_fase, tol)
% pause(1.0);
% converged = false;
% while ~converged
%     setpv('AON11FASE_SP', ref_fase);
%     pause(1.0);
%     fase= getpv('AON11FASE_AM');
%     converged = (abs(fase - ref_fase) < tol); 
% end
% pause(1.0);
% converged = false;
% while ~converged
%     setpv('AON11GAP_SP', ref_gap);
%     pause(1.0);
%     gap = getpv('AON11GAP_AM');
%     converged = (abs(gap - ref_gap) < tol);
%     %converged = true;
% end
% pause(1.5);
